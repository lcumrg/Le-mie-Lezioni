...
export default function ImpostazioniPage() {
  ...
  async function handleConfirmImport() { if (!importPreview || !annoAttivo) return; setImporting(true); try { const oreConfig = annoConfig?.oreLezione || null; const summary = await importToFirestore(importPreview, annoAttivo, oreConfig); const parts = []; if (summary.classi) parts.push(`${summary.classi} classi`); if (summary.orario) parts.push(`${summary.orario} slot orario`); if (summary.percorsi) parts.push(`${summary.percorsi} percorsi`); if (summary.unita) parts.push(`${summary.unita} unita`); if (summary.ricorrenze) parts.push(`${summary.ricorrenze} ricorrenze`); if (summary.vacanze) parts.push(`${summary.vacanze} vacanze`); toast.success(`Importazione completata: ${parts.join(', ')}`); if (summary.ricorrenzeScartate) toast.warning(`${summary.ricorrenzeScartate} ricorrenze scartate: citano percorsi assenti dal foglio "Percorsi e Unità" dello stesso file`); setImportPreview(null) } catch (err) { toast.error('Errore durante l\'importazione: ' + err.message) } finally { setImporting(false) } }
        ...
        <section id="import" className="bg-surface rounded-sm border border-edge p-6 scroll-mt-4">
          ...
          {importPreview && (
            <div className="border border-edge bg-overlay rounded-sm p-4 space-y-3">
              ...
              {importPreview.orario.length > 0 && !hasOreConfig && (
                <p className="text-xs text-warn bg-badge-warn border border-warn/30 rounded-sm px-3 py-2">
                  Le <strong>Ore Scolastiche</strong> non sono ancora configurate: gli slot orario importati
                  useranno gli orari del file (o 08:00 come ripiego). Meglio configurarle prima, qui sopra.
                </p>
              )}
