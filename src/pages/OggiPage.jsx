...
const STATO_BADGE = {
  ...
  [STATO_LEZIONE.PARZIALE]: 'bg-badge-h text-warn',
...
export default function OggiPage() {
  ...
  const [lezioniWeek, setLezioniWeek] = useState([])
  const [lezioniPrevWeek, setLezioniPrevWeek] = useState([])
  const [showFineAnno, setShowFineAnno] = useState(false)
  // Curriculum caricato per la sottoscrizione corrente (vedi SettimanaPage)
  const [datiPronti, setDatiPronti] = useState({ chiave: null, fonti: new Set() })
  ...
  useEffect(() => {
    ...
    const chiaveSub = `${annoAttivo}|${dayOffset}`
    const segnaPronto = (fonte) =>
      setDatiPronti((p) => {
        const fonti = p.chiave === chiaveSub ? new Set(p.fonti) : new Set()
        fonti.add(fonte)
        return { chiave: chiaveSub, fonti }
      })
    unsubs.push(onVacanze(annoAttivo, (data) => { setVacanze(data); segnaPronto('vacanze') }))
    unsubs.push(onRicorrenze(annoAttivo, (data) => { setRicorrenze(data); segnaPronto('ricorrenze') }))
    unsubs.push(onDistribuzioni(annoAttivo, (data) => { setDistribuzioni(data); segnaPronto('distribuzioni') }))

    return () => unsubs.forEach((u) => u())
  }, [annoAttivo, dayOffset])

  const generazionePronta =
    datiPronti.chiave === `${annoAttivo}|${dayOffset}` &&
    ['vacanze', 'ricorrenze', 'distribuzioni'].every((f) => datiPronti.fonti.has(f))
  ...
  useEffect(() => {
    ...
    const unsubs = []
    unsubs.push(onLezioniSettimana(annoAttivo, wStart, wEnd, setLezioniWeek))
    ...
    if (today.getDay() === 1) {
      ...
      const pwEnd = addDays(wStart, -1)
      unsubs.push(onLezioniSettimana(annoAttivo, pwStart, pwEnd, setLezioniPrevWeek))
    }
    return () => unsubs.forEach((u) => u())
  }, [annoAttivo])
  ...
  const parziali = lezioniOggi.filter((l) => l.stato === STATO_LEZIONE.PARZIALE).length
  ...
  const progressPct = totale > 0 ? Math.round(((svolte + parziali * 0.5) / totale) * 100) : 0
  ...
  // Auto-generate lessons when conditions are met — mai per giorni passati
  // (sfogliare ieri non deve creare lezioni retroattive) e solo a curriculum
  // caricato (evita la race con la cache: lezioni senza percorso/unità)
  useEffect(() => {
    if (loading || !annoAttivo || generating) return
    if (dayOffset < 0 || !generazionePronta) return
    if (!hasUngenerated) return
    if (autoGenerated.has(dayStr)) return
    setAutoGenerated((prev) => new Set(prev).add(dayStr))
    handleGenerate()
  }, [loading, annoAttivo, generating, dayOffset, generazionePronta, totale, slotsOggi.length, dayStr])
    ...
    try {
      ...
      if (prevStato && prevStato !== nuovoStato) {
        ...
        if (nuovoStato === STATO_LEZIONE.SALTATA && lez) {
          ...
          // Chiusura del loop: se lezione svolta e unità è ancora "da fare", propone di avanzarla
          if (nuovoStato === STATO_LEZIONE.SVOLTA && lez?.unitaId && lez?.percorsoId) {
            const unita = unitaMap[lez.unitaId]
            if (unita?.stato === STATO_UNITA.DA_FARE) {
              setTimeout(() => {
                toast.action(`Avanzare "${unita.titolo}" a In corso?`, {
                  label: 'Avanza',
                  onClick: () => updateUnita(lez.percorsoId, lez.unitaId, { stato: STATO_UNITA.IN_CORSO }),
                })
              }, 300)
            }
          }
  ...
  // ── Cosa manca ──
  const saltateWeek = lezioniWeek.filter((l) => l.stato === STATO_LEZIONE.SALTATA).length

  // ── Recap del lunedì ──
  const isMondayToday = dayDate.getDay() === 1 && dayOffset === 0
  const recapPrevWeek = isMondayToday && lezioniPrevWeek.length > 0 ? {
    totale: lezioniPrevWeek.length,
    svolte: lezioniPrevWeek.filter((l) => l.stato === STATO_LEZIONE.SVOLTA).length,
    parziali: lezioniPrevWeek.filter((l) => l.stato === STATO_LEZIONE.PARZIALE).length,
    saltate: lezioniPrevWeek.filter((l) => l.stato === STATO_LEZIONE.SALTATA).length,
  } : null
  ...
  const fineAnnoRows = assegnazioni
    ...
    .map((a) => ({ classe: a.classe, materia: a.materia, oreRimaste: computeOreRimaste(a.classe, a.materia) }))
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {/* -- Cosa manca: saltate questa settimana -- */}
      {dayOffset === 0 && !vacanza && saltateWeek > 0 && (
        <div className="mb-4 px-4 py-3 bg-badge-x border border-danger/30 rounded-sm flex items-center justify-between gap-3">
          <p className="text-sm text-danger font-medium">
            {saltateWeek === 1 ? '1 lezione saltata' : `${saltateWeek} lezioni saltate`} questa settimana
          </p>
          <button
      ...
      {recapPrevWeek && (
        <div className="mb-4 px-4 py-3 bg-surface border border-edge rounded-sm">
          <p className="text-xs font-semibold text-fg-subtle uppercase tracking-wide mb-1.5">Settimana scorsa</p>
          <div className="flex items-center gap-4 text-sm">
            <span className="text-accent font-mono font-semibold">{recapPrevWeek.svolte} svolte</span>
            {recapPrevWeek.parziali > 0 && (
              <span className="text-warn font-mono">{recapPrevWeek.parziali} parz.</span>
            )}
            {recapPrevWeek.saltate > 0 ? (
              <span className="text-danger font-mono font-semibold">{recapPrevWeek.saltate} saltate</span>
            ) : (
              <span className="text-fg-subtle font-mono">nessuna saltata</span>
            )}
            <span className="text-fg-subtle text-xs">su {recapPrevWeek.totale}</span>
          </div>
        </div>
      )}
        ...
        <div className="mb-4 px-4 py-3 bg-surface border border-edge rounded-sm">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3 text-sm text-fg-muted flex-wrap">
              ...
              {parziali > 0 && <span className="text-warn font-mono">{parziali} parziali</span>}
      ...
      {/* -- Widget fine anno -- */}
      {settimaneRimaste !== null && (
        <div className="mb-4">
          <button
            onClick={() => setShowFineAnno((v) => !v)}
            className="w-full flex items-center justify-between px-4 py-3 bg-surface border border-edge rounded-sm hover:bg-overlay transition-colors"
          >
            <span className="text-sm font-medium text-fg">
              Fine anno ·{' '}
              <span className={`font-mono font-bold ${settimaneRimaste <= 4 ? 'text-danger' : settimaneRimaste <= 8 ? 'text-warn' : 'text-link'}`}>
                {settimaneRimaste} settimane
              </span>
              {' '}rimaste
            </span>
            <svg
              className={`w-4 h-4 text-fg-muted transition-transform ${showFineAnno ? 'rotate-180' : ''}`}
              fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}
            >
              <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
            </svg>
          </button>
          {showFineAnno && fineAnnoRows.length > 0 && (
            <div className="border border-t-0 border-edge rounded-b-sm bg-surface divide-y divide-edge-muted">
              {fineAnnoRows.map((r) => (
                <div key={`${r.classe}|${r.materia}`} className="flex items-center justify-between px-4 py-2.5">
                  <span className="text-sm font-medium text-fg">
                    {r.classe}
                    <span className="text-xs text-fg-muted ml-1.5">{r.materia}</span>
                  </span>
                  <span className={`text-sm font-mono font-bold ${r.oreRimaste === 0 ? 'text-fg-subtle' : r.oreRimaste <= 5 ? 'text-danger' : r.oreRimaste <= 10 ? 'text-warn' : 'text-accent'}`}>
                    {r.oreRimaste}h disponibili
                  </span>
                </div>
              ))}
              <div className="px-4 py-2 text-xs text-fg-subtle flex justify-end">
                <button onClick={() => navigate('/programmazione?view=panoramica')} className="hover:text-link hover:underline">
                  Margine per classe →
                </button>
              </div>
            </div>
      ...
      {/* -- Auto-generate banner (fallback if auto-generate fails) -- */}
      {hasUngenerated && (
        <div className="mb-4 px-4 py-3 bg-badge-p border border-link/30 rounded-sm flex items-center justify-between gap-3">
          <p className="text-sm text-link">
            {generating ? 'Generazione lezioni in corso...' : <>Hai <strong>{slotsOggi.length}</strong> ore oggi non ancora generate.</>}
          </p>
          {!generating && (
            <button
      ...
      {/* -- "Segna tutte svolte" button (top, Phase 4.1) -- */}
      {!vacanza && dayOffset <= 0 && daFare > 0 && totale > 0 && (
        <button
          onClick={handleMarkAllSvolte}
          className="w-full mb-4 py-2.5 bg-badge-s border border-accent/30 text-accent rounded-sm font-medium text-sm hover:bg-badge-s/80 transition-colors flex items-center justify-center gap-2"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
          </svg>
          Segna tutte svolte ({daFare})
        </button>
      )}
                ...
                className={`rounded-sm border bg-surface transition-all ${
                  inCorso
                    ...
                    : lez.stato === STATO_LEZIONE.SVOLTA
                      ...
                      : lez.stato === STATO_LEZIONE.PARZIALE
                        ? 'border-warn/30 bg-badge-h/30'
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className={`w-0.5 self-stretch rounded-full shrink-0 min-h-[2.5rem] ${
                      ...
                      lez.stato === STATO_LEZIONE.PARZIALE ? 'bg-warn' :
                          ...
                          className={`w-8 h-8 rounded-sm text-xs font-bold flex items-center justify-center transition-all ${
                            lez.stato === s
                              ? `${STATO_BADGE[s]} ring-2 ring-offset-1 ring-offset-surface ${
                                  ...
                                  s === STATO_LEZIONE.SVOLTA ? 'ring-accent/40' :
                                  s === STATO_LEZIONE.PARZIALE ? 'ring-warn/40' : 'ring-danger/40'
                                }`
                  ...
                  <div className="mt-2 ml-[5.5rem]">
                    {percorso && !isEditingPercorso ? (
                      <div className="flex items-center gap-2">
                        <button
                          onClick={(e) => { e.stopPropagation(); navigate(`/percorsi?expand=${lez.percorsoId}`) }}
                          className="flex items-center gap-1.5 px-2 py-1 bg-badge-special border border-special/30 rounded-sm max-w-full hover:bg-special/20 transition-colors cursor-pointer"
                          title="Apri percorso"
                        >
