...
export default function SettimanaPage() {
  ...
  // Curriculum caricato per la sottoscrizione corrente: evita di generare
  // lezioni prima che vacanze/ricorrenze/distribuzioni siano arrivate
  ...
  useEffect(() => {
    ...
    const chiaveSub = `${annoAttivo}|${weekOffset}`
  ...
  const generazionePronta =
    datiPronti.chiave === `${annoAttivo}|${weekOffset}` &&
  ...
  // La rigenerazione elimina stati e note della settimana: chiede conferma
  ...
  // Auto-generate in list mode — mai per settimane passate, e solo quando
  // vacanze/ricorrenze/distribuzioni sono arrivate (evita lezioni senza
  // curriculum o in giorni di vacanza per colpa della race con la cache)
  useEffect(() => {
    if (viewMode !== 'list' || loading || !annoAttivo || annoChiuso || generating) return
    if (weekOffset < 0 || !generazionePronta) return
    ...
    handleGenerate()
  }, [viewMode, loading, annoAttivo, annoChiuso, weekOffset, generazionePronta, lezioni.length, orari.length, start, generating])
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <ConfirmDialog
        ...
        title="Rigenera la settimana"
        message={`Le ${lezioni.filter((l) => !l.extra).length} lezioni non-extra di questa settimana verranno eliminate e ricreate dalla timeline: stati (Svolta/Parziale/Saltata) e note andranno persi. Le lezioni extra restano.`}
